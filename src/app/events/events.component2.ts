...
export class EventsComponent implements OnInit, AfterContentChecked {
  ...
  public listEvents ( USER_ID?: string, demo?: boolean ) {
    ...
    if ( demo ) {
      ...
      // Observable.from( Object(MINI_EVENTS) )
      const events = this.request.getJSON( '/assets/data/events-mini' )
        .subscribe( data => {
         ...
         // .subscribe( data => console.log( 'evtjson', data ) );
        /*.subscribe(
          evts => {
